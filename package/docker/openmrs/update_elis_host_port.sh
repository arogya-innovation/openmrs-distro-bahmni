...
run_sql() {
  mysql -N -s --host="${OMRS_DB_HOSTNAME}" --port="${OMRS_DB_PORT}" --user="${OMRS_DB_USERNAME}" --password="${OMRS_DB_PASSWORD}" "${OMRS_DB_NAME}" -e "$1"
}

count=$(run_sql "select count(*) from information_schema.tables where table_schema='${OMRS_DB_NAME}' and table_name='markers';")
